...
HashSet<Point2D> asteroids = new HashSet<Point2D>();
...
{
    Point2D eliminated = map.Where(x => x.Value == (0, -1, 0)).OrderBy(x => (basePoint - x.Key).TaxiCabLength).First().Key;
...
while (vaporizedCount < 200)
{
    ...
    lastFull = map.Where(x => x.Value == next).OrderBy(x => (basePoint - x.Key).TaxiCabLength).First().Key;
...
bool CanSee(Point2D start, Point2D end)
{
    ...
    int gcd = AoCMath.GCD(Math.Abs(deltaX), Math.Abs(deltaY));
