...
Square[,] grid;
Dictionary<Point2D, int> movementGrid;
Dictionary<Point2D, char> keyDict = new Dictionary<Point2D, char>();
Dictionary<Point2D, char> doorDict = new Dictionary<Point2D, char>();
Dictionary<char, Point2D> reverseKeyDict = new Dictionary<char, Point2D>();
int allKeysMask;
char[] allKeys;

Dictionary<(char, char), int> keyDistances = new Dictionary<(char, char), int>();
Point2D startingPoint = Point2D.Zero;
Dictionary<(char lastKey, int keyState), int> bestKeyRemainder = new Dictionary<(char lastKey, int keyState), int>();


Dictionary<char, int> keyReq = new Dictionary<char, int>();
Dictionary<char, int> keyAssignment = new Dictionary<char, int>();
Point2D[] robotStartingPoints = new Point2D[4];
Dictionary<QuadState, int> bestQuadKeyRemainder = new Dictionary<QuadState, int>();

QuadState.keyReq = keyReq;
QuadState.keyAssignment = keyAssignment;
...
Console.WriteLine($"The robust answer is: {GetShortestPathFromState("1234")}");
...
int GetShortestPath(char lastKey, int keys)
{
...
int GetShortestPathFromState(in QuadState lastKeys)
{
    ...
    {
        shortestDistance = Math.Min(shortestDistance, keyDistances[(lastKeys[keyAssignment[key]], key)] + GetShortestPathFromState(lastKeys.GetWith(key)));
    }
...
readonly struct QuadState
{
    ...
    public static Dictionary<char, int> keyReq = null;
    public static Dictionary<char, int> keyAssignment = null;
