...
Console.WriteLine("Day 03 - Crossed Wires");
...
int closestIntersection = totalDelays.Select(x => x.point.TaxiCabLength).OrderBy(x => x).First();
...
static Point2D ParsePoint(string segment)
{
    char dir = segment[0];
    int number = int.Parse(segment[1..]);
